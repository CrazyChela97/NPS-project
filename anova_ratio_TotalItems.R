...
#stiamo facendo ANOVA, posso anche non togliere i dati del 2015
dim(cleandata[which(cleandata$Year=="2015"),])[1] #1411 osservazioni nel 2015
...
summary(fit) #TotalItems indipendente da EventType usando ANOVA che pero' non possiamo usare perche ipotesi violate
...
p_val  #0.228: TotalItems indipendente da EventType

#####---------- 
##ANOVA USA DATASET 

##---- TotalItems ~ EventType, Seasonality, weekend, ecc 
#proviamo solo negli USA
USA=cleandata[which(cleandata$Country=='USA'),] #26549 dati su 40364 totali

LandCleanup=USA[which(USA$EventType=='Land (beach, shoreline and inland) Cleanup'),] #23961
...
#cosa a random: nel 2015 sono tutti LandCleanup
...
p_val <- sum(T_stat>=T0)/B
p_val #0.016: NEGLI USA EventType E' SIGNIFICATIVO con alpha=0.05 o 0.1
...
T0_EventType_seasonality <- summary.aov(aov(USA$TotalItems ~ seasonality*EventType))[[1]][3,4]
...
for(perm in 1:B){
  ...
  T_EventType_seasonality[perm] <- summary.aov(aov(TotalItems.perm.H0EventType_seasonality ~ seasonality*EventType))[[1]][3,4]
}

sum(T_EventType_seasonality >= T0_EventType_seasonality)/B #0.075
...
sum(T_EventType >= T0_EventType)/B #0.026 EventType significant con alpha=0.1 o 0.05 

#abbiamo una riconferma, quindi con alpha=0.1 abbiamo TotalItems ~ EventType + EventType:seasonality
...
sum(T_month_year >= T0_month_year)/B #0.038 month_year significant se fissiamo alpha=0.05 o 0.1
...
T0_seasonality_year <- summary.aov(aov(USA$TotalItems ~ Year*seasonality))[[1]][3,4]
T0_seasonality_year
...
sum(T_seasonality_year >= T0_seasonality_year)/B #0.072 seasonality_year significant per alpha= 0.1
...
sum(T_seasonality >= T0_seasonality)/B #0 seasonality significant per alpha=0.05 o 0.1 
sum(T_year >= T0_year)/B #0.002 year significant con alpha=0.1 o 0.05
...
## RATIO ~ ~ EventType, Seasonality, weekend, ecc 
...
p_val <- sum(T_stat>=T0)/B
p_val #0.04 EventType con ratio significativo con alpha  0.05 e 0.1
...
#NON SERVE: faccio il modello completo usando *
summary.aov(aov(USA$ratio ~ seasonality*EventType))

# Without interaction
summary.aov(aov(USA$ratio ~ seasonality + EventType))

T0_EventType_seasonality <- summary.aov(aov(USA$ratio ~ seasonality*EventType))[[1]][3,4]
T0_EventType_seasonality
...
sum(T_EventType_seasonality >= T0_EventType_seasonality)/B #0.163
...
sum(T_seasonality >= T0_seasonality)/B #0.005 seasonality significant con alpha=0.1 o 0.05

sum(T_EventType >= T0_EventType)/B #0.066 EventType significant con alpha=0.1 o 0.05 
...
T0_month_year <- summary.aov(aov(USA$ratio ~ Year*Month))[[1]][3,4]
T0_month_year
...
sum(T_month_year >= T0_month_year)/B #0.025 month_year significant se fissiamo alpha=0.05 o 0.1
...
p_val <- sum(T_stat>=T0)/B
p_val #0.403 Year NON significativa.
...
p_val <- sum(T_stat>=T0)/B
p_val #0.133 Month NON significativa.
...
sum(T_seasonality_year >= T0_seasonality_year)/B #0.8 seasonality_year NON significant 
...
sum(T_year >= T0_year)/B #0.279 year not significant 
